...
public:
    ...
    double calcCustomNoon(double) const;
...
private:
    ...
    double calcHourAngleNoon(double, double, double) const;
    ...
    double calcAbsNoon(double) const;
...
double SunSet::calcAbsNoon(double offset) const
{
    ...
    double  delta = m_longitude+hourAngle ;
...
double SunSet::calcNoon() const
{   
    return calcCustomNoon(SUNSET_OFFICIAL);
...
double SunSet::calcCustomNoon(double angle) const
{
    return calcAbsNoon(angle)+(60*m_tzOffset);
}
...
int main(){
    ...
    double zone;
    cin >> zone;
    sun.setPosition(lat,lon,zone);
    sun.setCurrentDate(year,month,day);
    sun.setTZOffset(zone);
    double sunrise=sun.calcSunrise();
    double sunset=sun.calcSunset();
    double noon=sun.calcNoon();
    int moonphase=sun.moonPhase(time(nullptr));
    cout << "Solar Noon=" << formatTime(noon*60.0+6) <<endl;
    cout << "   Sunrise=" << formatTime(sunrise*60.0) << endl;
    cout << "    Sunset=" << formatTime(sunset*60.0) << endl;
...
2022 8 15
121.498146 31.243333
8.0

2022 8 15
73.6138916 39.264157
8.0

2022 12 22
123.277587 53.555198
8.0
