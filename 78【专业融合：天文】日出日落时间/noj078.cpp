...
void calculateSunriseSunset(double longitude, double latitude, int year, int month, int day) {
...
int main() {
    ...
    // 调用 C++ 版本的计算函数
    calculateSunriseSunset(lon, lat, year, month, day);

    //int sunrise = static_cast<int>(sunriseTime);
    //int sunset = static_cast<int>(sunsetTime);

    cout << "   Sunrise=" << sunriseTime << endl;
    cout << "    Sunset=" << sunsetTime << endl;
